...
class ClickHouseLoader:
    ...
    def insert_data(self, df: pd.DataFrame, table_name: str) -> None:
        ...
        """
        if df.empty:
            logger.warning(f"[LOAD] DataFrame kosong. Melewati proses insert untuk tabel '{table_name}'.")
            return

        try:
            logger.info(f"[LOAD] Memulai proses pengiriman {len(df)} baris ke tabel '{table_name}'...")
